...
parser.add_option("-d", "--add-domain", dest="dom", action="store_true", help="Automatically add the clients domain to Satellite6")
...
def create_host():
    ...
    if DOMAIN:
        ...
        try:
            ...
            print_generic("Domain %s doesn't exist in Satellite6, consider using the --add-domain option." % DOMAIN)
            sys.exit(2)
    else:
        mydomainid = None
